...
`pipeline_order`, `pdr_mod_additive`, `true_damage_pre_reduction`,
`scaling_ignored` (with the resolution pipeline and the damage model) and
`confidence_not_propagated` (with `Confidence<T>`).
